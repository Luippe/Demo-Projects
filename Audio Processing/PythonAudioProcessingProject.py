try:
    # PyAudioWPatch exposes Windows WASAPI loopback devices while keeping the
    # regular PyAudio API used by the rest of this project.
...
try:
    mic_input_info = p.get_default_input_device_info()
except OSError:
    mic_input_info = None
...
def get_pc_input_info():
    """Return the default speaker loopback, or a hardware system-mix input."""
    ...
    # Some audio drivers expose their own loopback capture device to regular
    # PyAudio, usually under one of these names.
    loopback_names = ('stereo mix', 'what u hear', 'waveout mix',
                      'wave out mix', 'mixed output', 'output monitor')
...
pc_input_info = get_pc_input_info()


def open_input_stream(device_info):
    return p.open(
        format=FORMAT,
...
# Prefer the microphone, but still let the app start on a machine whose default
# microphone is disconnected or disabled when PC loopback audio is available.
try:
    if mic_input_info is None:
        raise OSError('No default microphone')
    stream = open_input_stream(mic_input_info)
    input_info = mic_input_info
    input_source = 'mic'
except (OSError, ValueError):
    if pc_input_info is None:
        raise
    stream = open_input_stream(pc_input_info)
    input_info = pc_input_info
    input_source = 'pc'
CHANNELS_INPUT = int(input_info['maxInputChannels'])


def switch_input_source(new_source):
    """Switch the live input stream without dropping the current one on error."""
    global stream, input_info, CHANNELS_INPUT, input_source

    new_input_info = mic_input_info if new_source == 'mic' else pc_input_info
    source_name = 'Mic' if new_source == 'mic' else 'PC'
    if new_input_info is None:
        return False, f'{source_name} audio is unavailable'

    new_channels = int(new_input_info['maxInputChannels'])
    try:
        new_stream = open_input_stream(new_input_info)
    except (OSError, ValueError):
        return False, f'Could not open {source_name} audio'

    old_stream = stream
    stream = new_stream
    input_info = new_input_info
    CHANNELS_INPUT = new_channels
    input_source = new_source
    old_stream.stop_stream()
    old_stream.close()
    return True, f'{source_name} audio active'
...
class Pages():
    def __init__(self):
        ...
        self.input_status = ''
        self.input_status_timer = 0
        self.input_font = pygame.font.Font('editundo.ttf', 42)
        for num in range(2):
    ...
    def update(self):
        if self.run == True:
            ...
            if self.page == 0:
                ...
                input_x = 1800 - self.page_x
                draw_text('Input', self.font, BLACK, (input_x, 510))
                input_button = pygame.Rect(input_x, 590, 320, 70)
                pygame.draw.rect(screen, LIGHT_GREY, input_button, 0, 5)
                pygame.draw.rect(screen, WHITE, input_button, 4, 5)
                input_label = 'Mic' if input_source == 'mic' else 'PC'
                input_text = self.input_font.render(input_label, True, BLACK)
                screen.blit(input_text, input_text.get_rect(center=input_button.center))

                if (mouse_rect.colliderect(input_button)
                        and pygame.mouse.get_pressed()[0] == 1
                        and self.click_allow == True):
                    self.click_allow = False
                    new_source = 'pc' if input_source == 'mic' else 'mic'
                    _, self.input_status = switch_input_source(new_source)
                    self.input_status_timer = 180

                if self.input_status_timer > 0:
                    draw_text(self.input_status, font, WHITE, (input_x, 680))
                    self.input_status_timer -= 1
