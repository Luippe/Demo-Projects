...
# A data item may be either "filename" (bundled at the app root) or
...
DEMOS = [
    ...
    ("Solar System Simulation",   "orbit.py",                 "Solar-System-Simulation",   [
        ("orbit_images", "orbit_images"),
        "editundo.ttf",
        "orbitdata_hours",
    ]),
]
...
def build(folder, entry, name, data):
    ...
    for data_item in data:
        if isinstance(data_item, tuple):
            relative_path, destination = data_item
        else:
            relative_path, destination = data_item, "."
        src = os.path.join(demo_dir, relative_path)
        if not os.path.exists(src):
            return False, "data path not found: " + src
        cmd += ["--add-data", src + os.pathsep + destination]
