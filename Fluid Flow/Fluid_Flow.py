...
fps = 60
screen_size = 300
field = pygame.display.set_mode((screen_size, screen_size))
screen_width, screen_height = pygame.display.get_surface().get_size()
run = True

tile_size = 10
pixel_tile = screen_size / tile_size
source_pos_x = 15
source_pos_y = 15
source_vel_x = 0
source_vel_y = 0
...
def advect(d_array, v_x, v_y):
    for i in range(np.shape(d_array)[0] - 1):
        for j in range(np.shape(d_array)[1] - 1):
            ...
            if y_real < 0:
                y_real = 0
            if y_real > pixel_tile:
                y_real = pixel_tile
            if x_real < 0:
                x_real = 0
            if x_real > pixel_tile:
                x_real = pixel_tile
            i_init = int(x_real)
            i_fin = i_init + 1
            j_init = int(y_real)
...
while run:
    ...
    density_array_prev[source_pos_y][source_pos_x] = 500
    velocity_y[source_pos_y][source_pos_x] = source_vel_y
    velocity_x[source_pos_y][source_pos_x] = source_vel_x
